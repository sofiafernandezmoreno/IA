...
public:
	...
	// Piezas de cada jugador. piezas[J1][] para el jugador 1, piezas[J2][] para el jugador 2
	// piezas[x][i] es el número de piezas en la i-ésima casilla más cercana al granero de cada jugador,
	// de modo que piezas[J1][1] es el número de piezas que hay justo antes del granero del jugador 1
	// y piezas[J1][6] el número de piezas que hay en la casilla más alejada. Piezas[J1][0] son
	// las piezas que hay en el granero
...
//public:
	...
	bool isValidState() const; // Devuelve true si el estado actual del juego es válido
	Player getWinner() const; // Devuelve el jugador ganador de la partida, en caso de que sea estado final, o NONE en caso de que no sea estado final
	int getScore(Player p) const; // Devuelve la puntuación del jugador pasado por argumento. 0 si el jugador no es válido.
};
